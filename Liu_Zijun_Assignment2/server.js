...
app.post("/process_products", function (request, response,){
    POST = request.body;
...
/*
Tested, not enough time to fininsh, will revisit
app.post("/process_change", function (request, response) {
    // process a simple register form
    POST = request.body;
    
    change_username = POST["username"];
    console.log(change_username);
    change_username = change_username.toUpperCase();
    console.log(change_username);
 

        if(change_username == user_name_from_form){

            if(POST['password'] != ""){
            user_data[change_username].password = POST['password'];
            }    
    
            if(POST['email'] != ""){
            user_data[change_username].email = POST['email'];
            }

            if(POST['new_username'] != ""){
            user_data[change_username] = user_data[POST['new_username']];
            }
        ...
        response.send('Successfully changed information!');
        }
});
*/
